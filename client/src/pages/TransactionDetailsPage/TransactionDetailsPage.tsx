...
const ContractActionBadge: FC<{ contractActionName: string; actionName: string; }> = ({ contractActionName, actionName }) => {
...
const TransactionDetailsPage: FC = () => {
  const { id, loading, transaction, isIrreversible, stats, rawData } = useTransactionDetailsPageContext();
  const tabRowStyle = 'm-0 gap-4 d-flex flex-column flex-md-row flex-wrap w-100';

  return <Container className='py-5'>
    <BackButton to={ROUTES.transactions.path} />
    <h4>Transaction</h4>
      ...
      >
        ...
        <Tab eventKey="traces" title="Traces">
          {transaction?.traces.map((trace, key) => (
            ...
            >
              <ContractActionBadge contractActionName={trace.account_name} actionName={trace.action_name} />
